package com.tistory.ospace.simplefreemarker.configuration;
 ...
 * @since 3.2
 */
public class TilesView2 extends AbstractUrlBasedView {
	private static Logger logger = LoggerFactory.getLogger(TilesView2.class);

	@Nullable
	private Renderer renderer;

	private boolean exposeJstlAttributes = false;
	...
	public boolean checkResource(final Locale locale) throws Exception {
		...
		logger.info(">> checkResource begin: url[{}] locale[{}]", getUrl(), locale.toLanguageTag());
		...
		boolean isRenderable = this.renderer.isRenderable(getUrl(), request);
		
		logger.info(">> checkResource end: isRenderable[{}]", isRenderable);
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		logger.info(">> renderMergedOutputModel begin: model[{}]", model);
		Assert.state(this.renderer != null, "No Renderer set");
...
//		response.setCharacterEncoding("UTF-8");
		...
		logger.info(">> renderMergedOutputModel end");
	}
	...
	protected Request createTilesRequest(final HttpServletRequest request, HttpServletResponse response) {
		logger.info(">> createTilesRequest begin: locale[{}] applicationContext[{}]", RequestContextUtils.getLocale(request).toLanguageTag(), this.applicationContext.toString());
		Request ret = new ServletRequest(this.applicationContext, request, response) {
			@Override
			public Locale getRequestLocale() {
		...
		logger.info(">> createTilesRequest end: scopes[{}] context[{}]", ret.getAvailableScopes(), ret.getApplicationContext());
